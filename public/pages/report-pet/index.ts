...
const defaultReportImg = require('../../assets/default-report.svg');
...
class ReportPet extends HTMLElement {
	...
	addListeners() {
		...
		btnSaveEl?.addEventListener('click', () => {
			...
			console.log(reportPet.petName, reportPet.lat, reportPet.lng);
			if (reportPet.petName && reportPet.img && reportPet.lat && reportPet.lng) {
				state.addPet();
				console.log(`entre`);
			} else {
